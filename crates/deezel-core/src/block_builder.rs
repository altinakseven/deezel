...
//!
//! This module provides utilities for building Bitcoin blocks for testing,
//! following the patterns from the reference implementations.
...
/// Serialize a block to bytes for passing to runtime
pub fn serialize_block(block: &Block) -> Vec<u8> {
    use bitcoin::consensus::Encodable;
    let mut buffer = Vec::new();
    block.consensus_encode(&mut buffer).expect("Failed to encode block");
    buffer
}

/// Create input data for runtime (height + block bytes)
