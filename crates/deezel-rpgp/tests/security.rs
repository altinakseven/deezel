...
fn rpg_015_cleartext_signed_message_from_armor_panic1() {
    ...
    let _ = deezel_rpgp::composed::CleartextSignedMessage::from_armor(bad_input);
}
...
fn rgp_015_cleartext_signed_message_from_string_panic1() {
    ...
    let _ = deezel_rpgp::composed::CleartextSignedMessage::from_string(bad_input);
}
...
>>>>>>>>>[ESSAGE\tw:::::::";
    ...
    let _ = deezel_rpgp::composed::CleartextSignedMessage::from_string(data);
}
...
fn rpg_015_cleartext_signed_message_from_armor_panic2() {
    ...
    let _ = deezel_rpgp::composed::CleartextSignedMessage::from_armor(&data[..]);
}
...
fn rpg_017_signed_public_key_as_unsigned_panic1() {
    ...
    let key = deezel_rpgp::composed::SignedPublicKey::from_bytes(bad_input).unwrap();
    // expected bug behavior:
...
fn rpg_021_signed_secret_key_encrypt_panic1() {
    ...
    let key = deezel_rpgp::composed::SignedSecretKey::from_bytes(bad_input).unwrap();
    ...
    //
    // crash also happens with deezel_rpgp::types::EskType::V3_4
    let _ciphertext = {
        key.encrypt(
            &mut rng,
            dummy_plaintext.as_slice(),
            deezel_rpgp::types::EskType::V6,
        )
    };
}
...
fn rpg_021_signed_secret_key_encrypt_panic2() {
    ...
    // note, this is non-deterministic, but does not matter for reproduction
    // let mut rng = rand::thread_rng();
    let key = deezel_rpgp::composed::SignedSecretKey::from_bytes(bad_input);
    assert!(key.is_err());
...
fn rpg_020_signed_secret_key_create_signature_panic1() {
    ...
    let res = deezel_rpgp::composed::SignedSecretKey::from_bytes(bad_input);
    assert!(res.is_err());
    ...
    //     &Password::empty(),
    //     deezel_rpgp::crypto::hash::HashAlgorithm::Sha256,
    //     dummy_data,
    // );
}
...
fn rpg_010_standalone_signature_subtract_with_overflow1() {
    ...
    // attempt to subtract with overflow
    let _ = deezel_rpgp::composed::StandaloneSignature::from_bytes(bad_input);
}
...
fn oom_signature_1() {
    ...
    let res = deezel_rpgp::composed::StandaloneSignature::from_bytes(&bad_input[..]);
    assert!(res.is_err());
}
