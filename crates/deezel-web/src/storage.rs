...
//! - **Namespacing**: All keys are prefixed with "deezel:" to avoid conflicts
...
/// [`deezel_common::StorageProvider`] trait. It uses the browser's localStorage
...
/// - Keys are prefixed with "deezel:" to avoid conflicts with other applications
...
impl WebStorage {
    ...
    ///
    /// Prefixes the provided key with "deezel:" to create a namespaced
    ...
    /// # Returns
    ///
    /// A prefixed key in the format "deezel:{key}"
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    /// # use deezel_web::storage::WebStorage;
    /// let storage = WebStorage::new();
    /// // This would return "deezel:wallet_data"
...
/// Implementation of the [`deezel_common::StorageProvider`] trait for web environments
///
...
impl deezel_common::StorageProvider for WebStorage {
    ...
    /// # Arguments
    ///
    /// * `key` - The key to read (will be automatically prefixed with "deezel:")
    ///
    /// # Returns
    ///
    ...
    /// # Arguments
    ///
    /// * `key` - The key to store under (will be automatically prefixed with "deezel:")
    ...
    /// # Arguments
    ///
    /// * `key` - The key to check (will be automatically prefixed with "deezel:")
    ///
    /// # Returns
    ///
    ...
    /// # Arguments
    ///
    /// * `key` - The key to delete (will be automatically prefixed with "deezel:")
    ///
    /// # Errors
    ///
    /// * [`DeezelError::Storage`] if localStorage is not available
    ...
    /// The returned keys have the "deezel:" namespace prefix removed.
    ///
    /// # Arguments
    ///
    /// * `prefix` - The prefix to match against (will be automatically prefixed with "deezel:")
    ///
    /// # Returns
    ///
    /// A vector of keys (without the "deezel:" prefix) that match the prefix
    ///
    /// # Errors
    ///
    /// * [`DeezelError::Storage`] if localStorage is not available
        ...
        for i in 0..length {
            if let Ok(Some(key)) = storage.key(i) {
                if key.starts_with(&full_prefix) {
                    // Remove the "deezel:" prefix to return the original key
                    if let Some(original_key) = key.strip_prefix("deezel:") {
