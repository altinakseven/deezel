...
//! use deezel_common::NetworkProvider;
//!
...
/// [`deezel_common::NetworkProvider`] trait. It uses the browser's fetch API
/// for making HTTP requests with full support for binary data and CORS.
///
/// # Configuration
///
/// - Uses CORS mode for cross-origin requests
/// - Sets a default user agent string "deezel-web/0.1.0"
...
impl WebNetwork {
    ...
    /// "deezel-web/0.1.0". The instance is ready to make HTTP requests
    /// immediately.
    ///
    /// # Examples
    ///
    ...
    pub fn new() -> Self {
        Self {
            user_agent: "deezel-web/0.1.0".to_string(),
        }
    }
...
/// Implementation of the [`deezel_common::NetworkProvider`] trait for web environments
///
...
impl deezel_common::NetworkProvider for WebNetwork {
    ...
    /// # Returns
    ///
    /// The user agent string, defaults to "deezel-web/0.1.0"
...
mod tests {
    ...
    fn test_user_agent() {
        let network = WebNetwork::new();
        assert_eq!(network.user_agent(), "deezel-web/0.1.0");
    }
}