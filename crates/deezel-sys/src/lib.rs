        ...
        let res: anyhow::Result<()> = match command {
            ...
            AlkanesCommands::Inspect {
                ...
                if raw {
                    ...
                    pretty_print_inspection_result(&result)?;
                }
                Ok(())
            }
...
fn pretty_print_inspection_result(result: &AlkanesInspectResult) -> anyhow::Result<()> {
...
fn pretty_print_fuzzing_results(fuzzing_results: &FuzzingResults, alkane_id: &deezel_common::alkanes::types::AlkaneId) -> anyhow::Result<()> {
    println!("\n=== FUZZING ANALYSIS ===");
    println!("Alkane: {}:{}", alkane_id.block, alkane_id.tx);
    println!();
    println!("📊 Total opcodes tested: {}", fuzzing_results.total_opcodes_tested);
    if fuzzing_results.opcodes_filtered_out > 0 {
        println!("🔍 Opcodes filtered out (undefined behavior): {}", fuzzing_results.opcodes_filtered_out);
    }
    println!("✅ Successful executions: {}", fuzzing_results.successful_executions);
    println!("❌ Failed executions: {}", fuzzing_results.failed_executions);
    println!("🎯 Implemented opcodes: {} total", fuzzing_results.implemented_opcodes.len());
