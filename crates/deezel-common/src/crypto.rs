//! # Deezel Common Cryptography
//!
//! This module provides `no_std` compatible cryptographic functions for encrypting
//! and decrypting the wallet's seed mnemonic. It uses PBKDF2 to derive a key
//! from a user-provided passphrase and AES-256-GCM for authenticated encryption.

use crate::{Result, DeezelError};
use alloc::{vec, vec::Vec};
use aes_gcm::{
    aead::{Aead, KeyInit, OsRng},
    Aes256Gcm, Nonce,
};
use pbkdf2::pbkdf2_hmac;
use sha2::Sha256;
use rand_core::RngCore;

const KEY_SIZE: usize = 32; // 256 bits for AES-256
const SALT_SIZE: usize = 16; // 128 bits for salt
const NONCE_SIZE: usize = 12; // 96 bits for AES-GCM nonce
const PBKDF_ITERATIONS: u32 = 600_000; // A modern standard for PBKDF2 iterations

/// Encrypts data using a passphrase.
///
/// Derives a key from the passphrase using PBKDF2-HMAC-SHA256 and then encrypts
/// the data using AES-256-GCM.
///
/// # Returns
/// A tuple containing `(encrypted_data, salt, nonce)`.
pub fn encrypt(data: &[u8], passphrase: &str) -> Result<(Vec<u8>, Vec<u8>, Vec<u8>)> {
    // 1. Generate a random salt for PBKDF2
    let mut salt = vec![0u8; SALT_SIZE];
    OsRng.fill_bytes(&mut salt);

    // 2. Derive the encryption key from the passphrase and salt
    let mut key = vec![0u8; KEY_SIZE];
    pbkdf2_hmac::<Sha256>(
        passphrase.as_bytes(),
        &salt,
        PBKDF_ITERATIONS,
        &mut key,
    );

    // 3. Encrypt the data using AES-256-GCM
    let cipher = Aes256Gcm::new_from_slice(&key)
        .map_err(|e| DeezelError::Crypto(e.to_string()))?;
    let mut nonce_bytes = vec![0u8; NONCE_SIZE];
    OsRng.fill_bytes(&mut nonce_bytes);
    let nonce = Nonce::from_slice(&nonce_bytes);

    let encrypted_data = cipher.encrypt(nonce, data)
        .map_err(|e| DeezelError::Crypto(e.to_string()))?;

    Ok((encrypted_data, salt, nonce_bytes))
}

/// Decrypts data using a passphrase.
///
/// Re-derives the key from the passphrase and salt using PBKDF2-HMAC-SHA256
/// and then decrypts the data using AES-256-GCM.
///
/// # Returns
/// The decrypted plaintext data.
pub fn decrypt(encrypted_data: &[u8], passphrase: &str, salt: &[u8], nonce_bytes: &[u8]) -> Result<Vec<u8>> {
    // 1. Re-derive the encryption key from the passphrase and salt
    let mut key = vec![0u8; KEY_SIZE];
    pbkdf2_hmac::<Sha256>(
        passphrase.as_bytes(),
        salt,
        PBKDF_ITERATIONS,
        &mut key,
    );

    // 2. Decrypt the data using AES-256-GCM
    let cipher = Aes256Gcm::new_from_slice(&key)
        .map_err(|e| DeezelError::Crypto(e.to_string()))?;
    let nonce = Nonce::from_slice(nonce_bytes);

    let decrypted_data = cipher.decrypt(nonce, encrypted_data)
        .map_err(|e| DeezelError::Crypto(e.to_string()))?;

    Ok(decrypted_data)
}

#[cfg(test)]
mod tests {
    use super::*;
    use alloc::string::String;

    #[test]
    fn test_encrypt_decrypt_roundtrip() {
        let passphrase = "supersecretpassword";
        let data = "this is a very secret message";

        // Encrypt
        let (encrypted, salt, nonce) = encrypt(data.as_bytes(), passphrase).unwrap();

        // Decrypt
        let decrypted_bytes = decrypt(&encrypted, passphrase, &salt, &nonce).unwrap();
        let decrypted_string = String::from_utf8(decrypted_bytes).unwrap();

        assert_eq!(data, decrypted_string);
    }

    #[test]
    fn test_decrypt_wrong_password() {
        let passphrase = "supersecretpassword";
        let wrong_passphrase = "wrongpassword";
        let data = "this is another secret";

        // Encrypt
        let (encrypted, salt, nonce) = encrypt(data.as_bytes(), passphrase).unwrap();

        // Decrypt with wrong password
        let result = decrypt(&encrypted, wrong_passphrase, &salt, &nonce);

        assert!(result.is_err());
    }
}