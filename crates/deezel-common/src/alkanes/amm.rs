...
pub struct AmmManager<P: crate::traits::DeezelProvider> {
    rpc_client: Arc<RpcClient<P>>,
    _wallet_manager: Arc<WalletManager<P>>,
}

impl<P: crate::traits::DeezelProvider> AmmManager<P> {
    ...
    pub async fn create_pool(&self, params: PoolCreateParams) -> Result<TransactionResult> {
        ...
        if params.tokens.len() < 2 {
            return Err(crate::DeezelError::Validation("Pool creation requires at least 2 tokens".to_string()));
        }
