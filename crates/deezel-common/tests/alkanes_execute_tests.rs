...
async fn test_execute_single_transaction_success() {
    ...
    let funding_tx_out = TxOut {
        ...
        script_pubkey: Address::from_str(&WalletProvider::get_address(&provider).await.unwrap()).unwrap().require_network(Network::Regtest).unwrap().script_pubkey(),
    };
    provider.add_utxo(funding_outpoint, funding_tx_out);
